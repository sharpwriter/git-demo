package net.tpa
// This is a copy of a multi-jvm test available in the official Akka IO documentation
/*
...
import akka.actor.{Actor, ActorIdentity, ActorLogging, ActorPath, ActorRef, ActorSelection, Identify, Props, RootActorPath}
...
object ClusterSingletonManagerSpec extends MultiNodeConfig {
  ...
  val observer : RoleName = role("observer")
...
class ClusterSingletonManagerSpec extends MultiNodeSpec(ClusterSingletonManagerSpec) with STMultiNodeSpec with ImplicitSender {
  ...
  val controllerRootActorPath: ActorPath = node(controller)
